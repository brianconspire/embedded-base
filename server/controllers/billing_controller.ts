export async function checkBilling(ctx) {
  ...
  console.log("status");
  console.log(
    status.body.data.currentAppInstallation.activeSubscriptions.length
  );
  if (
    !(status.body.data.currentAppInstallation.activeSubscriptions.length > 0)
  ) {
    ...
    ctx.appSubscription =
      charge.body.data.appSubscriptionCreate.appSubscription.id;
